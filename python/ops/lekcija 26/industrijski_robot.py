...
def IndustrijskiRobot(proizvodjac, model, godina, sslk, potrosnja, sati, vp):
    if godina < 2000 or sslk < 1 or potrosnja < 1 or sati < 1:
        return -1
    ...
    return {
        ...
        "maksimalan_broj_sati_rada"         : sati,
        "vrsta_pogona_pokretnih_djelova"    : vp
        }
...
def efikasnost(ir):
    ...
    if ir['vrsta_pogona_pokretnih_djelova'] == "hidraulicni pogon":
        L = 1.33
    elif ir['vrsta_pogona_pokretnih_djelova'] == "elektromotorni pogon":
        L = 2.00
    else:
        L = 1.00

    return (ir['potrosnja_elektricne_energije'] * L) / ((ir['broj_stepeni_slobode_kretanja'] - 2) * math.sqrt(ir['maksimalan_broj_sati_rada']))
...
def load(imeDatoteke):
    ...
    while True:
        try:
            ...
            if (ir == -1):
                print("Doslo je do greske prilikom ucitavanja jedno industrijskog robota")
            else:
                lista.append(ir)
        except ValueError:
            break
